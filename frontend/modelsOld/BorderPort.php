...
class BorderPort extends \yii\db\ActiveRecord
{
    ...
    {
        return ArrayHelper::map(BorderPort::find()
            ->where(['location'=>1])
    ...
    {
        return ArrayHelper::map(BorderPort::find()->where(['location'=>2])->all(),'id','name');
    }
}
