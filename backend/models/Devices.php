...
//    }
    ...
    {
        $total = Devices::find()
            ->where(['not in', 'view_status', [Devices::fault_devices, Devices::damaged]])
            ->count();
        if ($total > 0) {
            echo $total;
        } else {
    ...
    {
        $total = Devices::find()
            ->where(['in', 'view_status', [Devices::fault_devices]])
            ->count();
        if ($total > 0) {
            echo $total;
        } else {
    ...
    {
        $total = Devices::find()
            ->where(['in', 'view_status', [Devices::in_transit]])
            ->count();
        if ($total > 0) {
            echo $total;
        } else {
    ...
    {
        $total = Devices::find()
            ->where(['in', 'view_status', [Devices::damaged]])
            ->count();
        if ($total > 0) {
            echo $total;
        } else {
    ...
    public function rules()
    {
        return [
            [['serial_no'], 'required'],
            [['received_from', 'border_port', 'received_from_staff', 'created_by',
                'status', 'device_from', 'stock_status', 'branch', 'type', 'released_by', 'released_to', 'transferred_from', 'transferred_to',
                'transferred_by', 'sales_person', 'sale_id', 'view_status', 'device_category', 'partiner','gate_number'], 'integer'],
            [['created_at', 'transferred_date', 'released_date'], 'safe'],
            [['remark'], 'string'],
            [['serial_no'], 'safe'],
