...
use frontend\models\BorderPortSearch;
...
class BorderPortController extends Controller
{
    ...
    {
        if (Yii::$app->user->can('viewBorder')){
            $searchModel = new BorderPortSearch();
    ...
    {
        if (Yii::$app->user->can('viewPortGate')) {
            $searchModel = new BorderPortSearch();
    ...
    {
        if (Yii::$app->user->can("newPortGate")) {
    ...
    public function actionUpdate($id)
    {
        ...
        if (Yii::$app->user->can("newBorder") || Yii::$app->user->can("newPortGate")) {
    ...
    {
        if (Yii::$app->user->can("deletePortGate") || Yii::$app->user->can("deleteBorder")) {

            BorderPort::updateAll(['status'=>0],['id'=>$id]);

            $username=Yii::$app->user->identity->username;
            Audit::setActivity('Border hii ' . $id . " imekuwa deleted na $username ", 'BorderPort', 'Update', '', '');
