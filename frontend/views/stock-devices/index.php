...
<div class="received-devices-index" style="padding-top: 2%">
    ...
    <?php if (Yii::$app->user->can('releaseDevices')) { ?>
        ...
        <?= Html::dropDownList("action", "", ArrayHelper::map(\backend\models\User::find()->where(['user_type' => \backend\models\User::PORT_STAFF])->andWhere(['role' =>"Sales"])->all(), 'id', 'username'), ['prompt' => '--- Sales Person ---','style'=>['height'=>'40px']]); ?>
        <?= Html::dropDownList("points", "", ArrayHelper::map(\backend\models\BorderPort::find()->where(['location' => 2])->all(), 'id', 'name'), ['prompt' => '--- Sales Point ---','style'=>['height'=>'40px']]); ?>
